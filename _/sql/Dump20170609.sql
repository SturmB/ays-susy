...
INSERT INTO `clipart_categories` VALUES (1,'aircraft'),(2,'watercraft'),(3,'theme');
...
CREATE TABLE `clipart_subcategories` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'PK.',
  `folder_name` varchar(45) NOT NULL COMMENT 'The short name of the clipart type. Typically lower-case only and one word, using hyphens to separate words.',
...
INSERT INTO `clipart_subcategories` VALUES (1,'aircraft - jets','Jets',1),(2,'aircraft - turboprop','Turbo Props',1),(3,'aircraft - multi-engine','Multi-Engine Planes',1),(4,'aircraft - single-engine','Single-Engine Planes',1),(5,'aircraft - helicopters','Helicopters',1),(6,'theme - clouds','Clouds',3),(7,'watercraft - motor yacht','Motor Yachts',2),(8,'watercraft - day cruiser','Day Cruisers',2),(9,'watercraft - speed boat','Speed Boats',2),(10,'watercraft - sail boat','Sail Boats',2),(11,'watercraft - ketch','Ketches',2),(12,'watercraft - sport fish','Sport Fish Boats',2),(13,'watercraft - house boat','House Boats',2),(14,'watercraft - pontoon','Pontoons',2),(15,'theme - nautical','Nautical-Themed',3);
...
CREATE TABLE `product_categories` (
  ...
  `priority` tinyint(4) NOT NULL DEFAULT '40' COMMENT 'Grouping and Priority in one column! This is for the navbar, with higher numbers appearing on the right. If a category shares the same number as another, their subcategories will be in the same menu.',
  `menu_name` varchar(45) NOT NULL COMMENT 'The menu name of this category. Ties together with the priority column before it.',
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`)
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `ACS_Menu_GetMenuItems` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`skyweb`@`%` PROCEDURE `ACS_Menu_GetMenuItems`(
	IN p_active VARCHAR(10)
)
BEGIN
	...
	SELECT c.menu_name as category_name,
			s.long_name as subcategory_name,
            c.short_name AS category_short,
            s.short_name AS subcategory_short,
            m.short_name AS method_short,
            MIN(m.id) AS method_id
		FROM product_subcategories as s
		JOIN product_categories as c
		ON s.category_id = c.id
		JOIN subcategoriesmethods as p
		ON p.subcategory_id = s.id
		LEFT JOIN print_methods as m
		ON p.method_id = m.id
		WHERE c.active IN (1, @active)
		AND s.active IN (1, @active)
...
BEGIN
	...
	SELECT id, coupon_id
		...
		WHERE subcategory_id = @subcat_id
			AND (CASE WHEN @method_id IS NULL
				THEN method_id IS NULL
                ELSE method_id = @method_id
			END)
            AND active IN (1, @active)
		LIMIT 1;
    ...
    SELECT id AS product_id, CONCAT(@prefix, item_num) AS product_name, description, @coupon_code AS coupon_code, @sm_id AS sm_id
		FROM Products
        WHERE id IN (
			SELECT product_id FROM ProductsMethods WHERE
				(CASE WHEN @method_id IS NULL
					THEN method_id IS NULL
					ELSE method_id = @method_id
				END)
				AND product_id IN (
...
-- Dump completed on 2017-06-09 16:30:01
